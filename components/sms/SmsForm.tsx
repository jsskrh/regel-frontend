...
export function SmsForm() {
  ...
  const campaignForm = useForm({
    ...
    defaultValues: { name: "", from: "", message: "", contactListId: "", phoneNumbers: "" },
  });
  ...
  useEffect(() => {
    if (activeTab === 'single') {
        ...
        const manualRecipients = campaignRecipients?.split(',').filter(Boolean).length || 0;
  ...
  const onCampaignSubmit = async (data: z.infer<typeof campaignSchema>) => {
    try {
      const phoneNumbers = data.phoneNumbers?.split(',').map(s => s.trim()).filter(Boolean) || [];
      await createCampaign({ createCampaignDto: {...data, phoneNumbers} }).unwrap();
  ...
  return (
    <div>
        <BalanceInfo score={smsCost} maxScore={accountDetails?.balance} amount={accountDetails?.balance} />
        <Tabs defaultValue="single" onValueChange={setActiveTab} className="mt-6">
            ...
            <TabsContent value="single">
                ...
                <CardContent className="p-4">
                    <Form {...singleSmsForm}>
                    <form onSubmit={singleSmsForm.handleSubmit(onSingleSmsSubmit)} className="space-y-4">
                        <FormField control={singleSmsForm.control} name="to" render={({ field }) => (
                            <FormItem>
                        ...
                        />
                        <FormField control={singleSmsForm.control} name="from" render={({ field }) => (
                            <FormItem>
                            <FormLabel>Sender ID (Optional)</FormLabel>
                            <FormControl><Input placeholder="Your Sender ID" {...field} /></FormControl>
                            <FormMessage />
                            </FormItem>
                        )}
                        />
                        <FormField control={singleSmsForm.control} name="message" render={({ field }) => (
                            <FormItem>
                        ...
                        />
                        <Button type="submit" disabled={isSendingSingle}>{isSendingSingle ? "Sending..." : "Send SMS"}</Button>
                    </form>
                    </Form>
                </CardContent>
                </Card>
                ...
                <CardContent className="p-4">
                    ...
                    <form onSubmit={campaignForm.handleSubmit(onCampaignSubmit)} className="space-y-4">
                        <FormField control={campaignForm.control} name="name" render={({ field }) => (
                            ...
                            <FormControl><Input placeholder="e.g. Summer Promo" {...field} /></FormControl>
                            <FormMessage />
                            </FormItem>
                        )}
                        />
                        <FormField control={campaignForm.control} name="from" render={({ field }) => (
                            <FormItem>
                            <FormLabel>Sender ID (Optional)</FormLabel>
                            <FormControl><Input placeholder="Your Sender ID" {...field} /></FormControl>
                            <FormMessage />
                        ...
                        />
                        <FormField control={campaignForm.control} name="contactListId" render={({ field }) => (
                        <FormItem>
                            ...
                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                ...
                                <SelectContent>
                                {contactLists.map(list => <SelectItem key={list._id} value={list._id}>{list.name}</SelectItem>)}
                                </SelectContent>
                            </Select>
                            <FormMessage />
                        </FormItem>
                        )} />
                        <FormField control={campaignForm.control} name="phoneNumbers" render={({ field }) => (
                            <FormItem>
                            <FormLabel>Or Enter Phone Numbers</FormLabel>
                            <FormControl><Textarea placeholder="Enter numbers separated by commas" {...field} /></FormControl>
                            <FormMessage />
                            </FormItem>
                        )}
                        />
                        <FormField control={campaignForm.control} name="message" render={({ field }) => (
                            <FormItem>
                            <FormLabel>Message</FormLabel>
                            <FormControl><Textarea {...field} /></FormControl>
                            <FormMessage />
                        ...
                        <Button type="submit" disabled={isCreatingCampaign}>{isCreatingCampaign ? "Creating..." : "Create Campaign"}</Button>
                    </form>
                    </Form>
                </CardContent>
                </Card>
